...
├── rxpro-manager         → task-tracker, state-writer, context-loader
├── rxpro-builder         → component-builder, data-layer-builder, page-assembler
...
1. Invoke `@rxpro-manager` → "Load full context from memory files"
...
3. **Delegation rules:**
   ...
   - Complex multi-file changes → `@rxpro-builder`
...
1. Invoke `@quality-gate` → run tsc + build
